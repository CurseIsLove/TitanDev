...
from naruto import naruto, setbot, Command, Owner, BotUsername, DB_AVAILABLE, AdminSettings, OwnerName, PM_PERMIT
...
@naruto.on_message(filters.me & filters.command(["revoke", "disapprove"], Command) & filters.private)
...
async def pm_button(client, query):
    if PM_PERMIT:
        ...
        if re.match(r"engine_pm_block", query.data):
            ...
            await naruto.send_message(query.from_user.id, "Opps, no nudes for you")
            await naruto.block_user(query.from_user.id)
        elif re.match(r"engine_pm_nope", query.data):
            await setbot.edit_inline_text(query.inline_message_id, "👍")
            await naruto.send_message(query.from_user.id,
            ...
            from naruto.modules.lydia import lydia_status
            if lydia_status:
                await naruto.send_message(query.from_user.id,
            ...
            await naruto.send_message(target, "Hello, this is titan, my master approved you to PM.")
            set_whitelist(int(target), True)
        elif re.match(r"engine_pm_blk", query.data):
            target = query.data.split("-")[1]
            await query.message.edit_text("That user was blocked~")
            await naruto.send_message(target, "Hello, this is titan, my master has decide to block you.\nSorry for this!")
