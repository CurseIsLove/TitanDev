...
from naruto import naruto, setbot, Command, DB_AVAILABLE, AdminSettings, edrep

if DB_AVAILABLE:
    from naruto.assistant.database.stickers_db import get_sticker_set, get_stanim_set
...
@naruto.on_message(filters.user(AdminSettings) & filters.command("kang", Command))
async def kang_stickers(client, message):
    ...
    if message.reply_to_message and message.reply_to_message.sticker:
        if message.reply_to_message.sticker.mime_type == "application/x-tgsticker":
            ...
            await client.download_media(
                message.reply_to_message.sticker, file_name="naruto/cache/sticker.tgs"
            )
        else:
            await client.download_media(
                message.reply_to_message.sticker, file_name="naruto/cache/sticker.png"
            )
    elif message.reply_to_message and message.reply_to_message.photo:
        await client.download_media(
            message.reply_to_message.photo, file_name="naruto/cache/sticker.png"
        )
    elif (
        ...
        await client.download_media(
            message.reply_to_message.document, file_name="naruto/cache/sticker.png"
        )
    else:
        await edrep(
            message,
    ...
    if (
        ...
        im = Image.open("naruto/cache/sticker.png")
        ...
        im.save("naruto/cache/sticker.png", "PNG")
    ...
    time.sleep(0.2)
    checkfull = await naruto.get_history("@Stickers", limit=1)
    if (
        checkfull[0].text
        ...
        os.remove("naruto/cache/sticker.png")
        return
    if (
        message.reply_to_message.sticker
        and message.reply_to_message.sticker.mime_type == "application/x-tgsticker"
    ):
        await client.send_document("@Stickers", "naruto/cache/sticker.tgs")
        os.remove("naruto/cache/sticker.tgs")
    else:
        await client.send_document("@Stickers", "naruto/cache/sticker.png")
        os.remove("naruto/cache/sticker.png")
