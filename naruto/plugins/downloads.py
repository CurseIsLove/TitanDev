...
@naruto.on_message(filters.user(AdminSettings) & filters.command("ls", Command))
async def ls(_, message):
    args = message.text.split(None, 1)
    basepath = "naruto/{}".format(args[1]) if len(args) == 2 else "naruto/"
    directory = "".join(
        "\n{}".format(entry)
...
@naruto.on_message(filters.user(AdminSettings) & filters.command("upload", Command))
async def upload_file(client, message):
    ...
    if len(args) == 1:
        ...
        return
    c_time = time.time()
    path = "naruto/{}".format(args[1])
    try:
        await naruto.send_document(
            message.chat.id,
            path,
...
async def download_url(url, file_name):
    ...
    try:
        os.rename(downloader.file_name, "naruto/downloads/" + file_name)
    except OSError:
        return "Failed to download file\nInvaild file name!"
    return downlaoded


@naruto.on_message(filters.user(AdminSettings) & filters.command("dl", Command))
...
@naruto.on_message(filters.user(AdminSettings) & filters.command("download", Command))
...
@naruto.on_message(filters.user(AdminSettings) & filters.command("direct", Command))
...
async def download_reply_nocall(client, message):
    if message.reply_to_message.photo:
        ...
        await client.download_media(
            message.reply_to_message.photo, file_name="naruto/downloads/" + nama
        )
        ...
        await client.download_media(
            message.reply_to_message.animation, file_name="naruto/downloads/" + nama
        )
        ...
        await client.download_media(
            message.reply_to_message.video, file_name="naruto/downloads/" + nama
        )
        ...
        await client.download_media(
            message.reply_to_message.sticker, file_name="naruto/downloads/" + nama
        )
    elif message.reply_to_message.audio:
        nama = "{}".format(message.reply_to_message.audio.file_name)
        await client.download_media(
            message.reply_to_message.audio, file_name="naruto/downloads/" + nama
        )
    elif message.reply_to_message.voice:
        nama = "audio_{}.ogg".format(message.reply_to_message.voice.date)
        await client.download_media(
            message.reply_to_message.voice, file_name="naruto/downloads/" + nama
        )
    elif message.reply_to_message.document:
        nama = "{}".format(message.reply_to_message.document.file_name)
        await client.download_media(
            message.reply_to_message.document, file_name="naruto/downloads/" + nama
        )
    else:
        return False
    return "naruto/downloads/" + nama
...
async def download_file_from_tg(client, message):
    ...
    if message.reply_to_message.photo:
        await client.download_media(
            ...
            file_name="naruto/downloads/" + name,
            progress=lambda d, t: client.loop.create_task(
                progressdl(d, t, message, c_time, "Downloading...")
            ),
        )
