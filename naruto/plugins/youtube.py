...
from naruto import naruto, setbot, Command, AdminSettings, edrep
from naruto.helpers.parser import escape_markdown
from naruto.plugins.downloads import download_url
...
@naruto.on_message(filters.user(AdminSettings) & filters.command("ytdl", Command))
async def youtube_download(_, message):
    ...
    if len(args) == 2:
        ...
        YouTube(link).streams.first().download("naruto/downloads", filename="tempvid")
        await naruto.send_video(message.chat.id, video="naruto/downloads/tempvid.mp4")
        os.remove("naruto/downloads/tempvid.mp4")
    ...
    if len(args) == 3:
        ...
        stream.download("naruto/downloads", filename="tempvid")
        await naruto.send_video(message.chat.id, video="naruto/downloads/tempvid.mp4")
        os.remove("naruto/downloads/tempvid.mp4")
        await edrep(message, text=text, disable_web_page_preview=True)
        return
...
@naruto.on_message(filters.user(AdminSettings) & filters.command("ytmusic", Command))
