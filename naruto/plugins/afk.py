...
Security
Insights
Nana-Remix/nana/plugins/afk.py
@pokurt
pokurt Nana-Remix v3.2 (#101)
…
 2 contributors
190 lines (171 sloc)  5.43 KB
...
@naruto.on_message(filters.me & (filters.command("afk", COMMAND_PREFIXES)))
async def afk(_, message):
    if not DB_AVAILABLE:
        await message.edit("Your database is not avaiable!")
        return
...
@naruto.on_message(filters.mentioned & ~filters.bot, group=11)
async def afk_mentioned(_, message):
    if not DB_AVAILABLE:
        return
    global MENTIONED
...
@naruto.on_message(filters.me & filters.group, group=12)
async def no_longer_afk(_, message):
    if not DB_AVAILABLE:
        return
    global MENTIONED
