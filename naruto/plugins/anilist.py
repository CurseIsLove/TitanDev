...
from naruto import app, Command, AdminSettings, BotUsername, edrep, Owner, setbot
...
async def add_favorite(_, query):
    if query.from_user.id in AdminSettings:
        ...
        if add:
            await query.answer("Added successfully to your favourites", show_alert=True)
        else:
            await query.answer("Anime already exists in your favourites", show_alert=True)
    else:
        await query.answer("You are not allowed to use Press this", show_alert=True)
