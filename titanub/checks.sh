...
_checkUnoffPlugins() {
    ...
    if test $LOAD_UNOFFICIAL_PLUGINS = true; then
        ...
        gitClone --depth=1 https://github.com/edguru/titan-plugs/blob/main.git
        editLastMessage "\t\tUpgrading PIP ..."
        upgradePip
        editLastMessage "\t\tInstalling Requirements ..."
        installReq titan-plugs
        editLastMessage "\t\tCleaning ..."
        rm -rf naruto/plugins/unofficial/
        mv titan-plugs/plugins/ naruto/plugins/unofficial/
        cp -r titan-plugs/resources/* resources/
        rm -rf titan_plugs/
        deleteLastMessage
